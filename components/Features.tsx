    ...
    <section className='flexCenter flex-col overflow-hidden bg-feature-bg bg-center bg-no-repeat py-24'>
      <div className='max-container padding-container relative w-full flex justify-end'>
        ...
        <div className='z-20 flex w-full flex-col lg:w-[60%]'>
          ...
          <motion.ul variants={container} whileInView="visible" initial="hidden" className='mt-10 grid gap-10 md:grid-cols-2 lg:mg-20 lg:gap-20'>
            {
              FEATURES.map((feature) => (
                <motion.div key={feature.title} variants={item}>
                  <FeatureItem key={feature.title} icon={feature.icon} variant={feature.variant} description={feature.description} title={feature.title} />
                </motion.div>
              ))
            }
          </motion.ul>
